...
def main():
    ...
    if args.dry_run:
        ...
        if cfg["docking_mode"] == "redock_native":
            lig = list(Path(cfg["paths"]["native_ligands_folder"]).glob("*.pdbqt"))
        else:
    ...
    if cfg["docking_mode"] == "redock_native" and existing_docked:
        ...
        # Wybór folderu z ligandami zależny od trybu dokowania
        if cfg["docking_mode"] == "redock_native":
            cfg["paths"]["active_ligands_folder"] = cfg["paths"]["native_ligands_folder"]
        elif cfg["docking_mode"] in {"matrix", "diagonal"}:
            cfg["paths"]["active_ligands_folder"] = cfg["paths"]["ligands_folder"]
        else:
            raise ValueError(f"Unsupported docking_mode: {cfg['docking_mode']}")
    ...
    ok = any("native_redock" in p.name for p in out_dir.glob("*.pdbqt"))
